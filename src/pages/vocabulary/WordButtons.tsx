...
const WordButtons = observer(
  ({ hardWordsId, easyWordsId }: { hardWordsId: string[]; easyWordsId: string[] }) => {
    const { vocabulary, store } = useContext(Context)

    return (
      <ul className={styles['word-buttons']}>
        {vocabulary.words.map((word) => (
          <li key={word._id} className={styles['word-button-list']}>
            <button
              className={`${styles['word-button']} ${
                word.id === vocabulary.word.id && styles['word-button_active']
              }`}
              onClick={() => vocabulary.setWord(word)}
            >
