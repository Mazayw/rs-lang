...
const Word = observer(
  ({
    ...
    const handlerClickAudio = () => {
      ...
      const audioPlayer = new Audio(`${SETTINGS.BASE_URL}/${vocabulary.word.audio}`)
      audioPlayer.play()
      audioPlayer.onended = function () {
        const audioPlayer = new Audio(`${SETTINGS.BASE_URL}/${vocabulary.word.audioMeaning}`)
        audioPlayer.play()
        audioPlayer.onended = function () {
          const audioPlayer = new Audio(`${SETTINGS.BASE_URL}/${vocabulary.word.audioExample}`)
          audioPlayer.play()
    ...
    const getUserWord = async () => {
      ...
      if (userId && token) {
        const dataResponse = await apiService.getUserWord(userId, vocabulary.word.id, token)
    ...
    return vocabulary.word.id || vocabulary.word._id ? (
      <>
        ...
        >
          <img
            src={`${SETTINGS.BASE_URL}/${vocabulary.word.image}`}
            alt='word'
          ...
          <div className={styles['word-card__content']}>
            ...
            <p className={styles['word-translate']}>
              {vocabulary.word.wordTranslate}
              <span className={styles['word-transcription']}>{vocabulary.word.transcription}</span>
            </p>
            <p
              className={styles['text-meaning']}
              dangerouslySetInnerHTML={{ __html: vocabulary.word.textMeaning }}
            ></p>
            <p className={styles['text-meaning-translate']}>
              {vocabulary.word.textMeaningTranslate}
            </p>
            <p
              className={styles['text-meaning']}
              dangerouslySetInnerHTML={{ __html: vocabulary.word.textExample }}
            ></p>
            <p className={styles['text-example-translate']}>
              {vocabulary.word.textExampleTranslate}
            </p>
          </div>
          <div
            ...
            >
              ...
              >
                <g id='#000000ff'>
                  <path
                    fill={
                      easyWordsId.includes(vocabulary.word.id || vocabulary.word._id)
                        ? '#F5443B'
                        : '#171836'
                    }
                    opacity='1.00'
