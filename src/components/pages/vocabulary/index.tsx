...
function Vocabulary({
  ...
  useEffect(() => {
    (async function () {
      ...
      let data: IWord[]
      if (
        sessionStorage.getItem('sectionButtonNumber') ||
        sessionStorage.getItem('pageButtonNumber')
      ) {
        data = (await apiService.getAllWords(
          sessionStorageSectionButton,
          sessionStoragePageButton,
        )) as IWord[]
  ...
  const handlerClickSectionButton = async (
    e: React.MouseEvent<HTMLButtonElement, MouseEvent>,
    index: number,
  ) => {
    setCheck20WordsInPage([])
  ...
  return (
    ...
    >
      ...
      <CreateTextbookSectionsButtons
        ...
        onClickSectionButton={handlerClickSectionButton}
        onClickSectionHardButton={handlerClickHardButton}
      />
      <Word
        {...word}
      ...
      <WordButtons
        words={words}
        buttonWordCurrentIndex={buttonWordCurrentIndex}
        clickWordButtons={handlerClickWordButtons}
        hardWordsId={hardWordsId}
        easyWordsId={easyWordsId}
        token={token}
      />
      <TextbookPagesButtons
        textbookNumberPage={textbookNumberPage}
        buttonSectionCurrentIndex={buttonSectionCurrentIndex}
        clickPrevPage={handlerClickPrevPage}
        clickNextPage={handlerClickNextPage}
