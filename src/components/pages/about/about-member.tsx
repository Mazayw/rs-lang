...
export function AboutMember(props: INickname) {
  ...
  return (
    <div className={styles['about__member']} onClick={() => window.open(`${aboutData[nickname].link}`, '_blank')}>
      <img className={styles['member__photo']} src={aboutData[nickname].photo} alt="member's team" />
      <div className={styles['member__about']}>
        <h3 className={styles['member__name']}>{aboutData[nickname].name}</h3>
        <div className={styles['member__separator']}></div>
